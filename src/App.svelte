<script lang="ts">
  ...
  async function loadImages() {
    ...
    try {
      ...
      const { readDir, readDirRecursive } = await import('@tauri-apps/api/fs');
      const entries = await readDirRecursive(imageFolder);
      const paths = entries
        .filter(e => !e.children && /\.(jpe?g|png|tiff?|bmp)$/i.test(e.path))
        .map(e => e.path);
      ...
      <tbody>
        {#each images as img, i}
          <tr>
            ...
            <td>
              <input class="qr" bind:value={img.qr_code} placeholder="Enter QR…" />
            </td>
            <td>{img.date}</td>
